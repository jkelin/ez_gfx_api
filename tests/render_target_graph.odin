...
described_render_target_binds_explicit_id_per_pipeline :: proc(t: ^testing.T) {
	...
	if !testing.expect(t, gfx.ez_gfx_begin_render(&app.window) == .Ok, "explicit target test failed to begin render") {
		return
	}
	explicit_width := u32(WIDTH / 2)
	explicit_height := u32(HEIGHT / 2)
	source_id, source_status := gfx.ez_gfx_render_target_describe(
		explicit_width,
		explicit_height,
	...
	if !testing.expect(t, gfx.ez_gfx_finish_render() == .Ok, "explicit target render failed to submit") {
		return
	}
	gfx.ez_gfx_ctx_wait_idle()
...
render_target_describe_requires_context :: proc(t: ^testing.T) {
	...
	_, status := gfx.ez_gfx_render_target_describe(64, 64, "offscreen")
...
render_target_describe_rejects_zero_size :: proc(t: ^testing.T) {
	...
	_, zero_width_status := gfx.ez_gfx_render_target_describe(0, 64, "offscreen")
	_, zero_height_status := gfx.ez_gfx_render_target_describe(64, 0, "offscreen")
...
render_target_describe_reuses_id_for_same_label_and_size :: proc(t: ^testing.T) {
	...
	first, first_status := gfx.ez_gfx_render_target_describe(320, 240, "shared offscreen")
	second, second_status := gfx.ez_gfx_render_target_describe(320, 240, "shared offscreen")
...
render_target_describe_bumps_generation_when_size_changes :: proc(t: ^testing.T) {
	...
	first, first_status := gfx.ez_gfx_render_target_describe(320, 240, "resizable offscreen")
	if !testing.expect(t, first_status == .Ok, "initial describe failed") do return
	resized, resized_status := gfx.ez_gfx_render_target_describe(640, 480, "resizable offscreen")
...
render_target_describe_stale_id_fails_pipeline_bind :: proc(t: ^testing.T) {
	...
	stale_id, stale_status := gfx.ez_gfx_render_target_describe(320, 240, "stale source target")
	if !testing.expect(t, stale_status == .Ok, "initial describe failed") do return
	if !testing.expect(t, gfx.ez_gfx_begin_render(&app.window) == .Ok, "stale id test failed to begin render") {
		return
	}
	_, resize_status := gfx.ez_gfx_render_target_describe(640, 480, "stale source target")
...
render_target_describe_init_ctx :: proc(app: ^Render_Target_Graph_App) -> bool {
	...
	return gfx.ez_gfx_ctx_init_device(app.window.surface) == .Ok
}
...
render_target_graph_add_pipeline :: proc(
	...
	if pipeline_status != .Ok do return false
