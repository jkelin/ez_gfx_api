...
vertex_allocator_reuses_first_fitting_chunk_and_splits_remainder :: proc(t: ^testing.T) {
	...
	first, first_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 16)
	second, second_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 8)
	testing.expect_value(t, first_status, gfx.Ez_Gfx_Status.Ok)
	testing.expect_value(t, second_status, gfx.Ez_Gfx_Status.Ok)
	testing.expect_value(t, first, vk.DeviceSize(0))
	testing.expect_value(t, second, vk.DeviceSize(16))

	freed_status := gfx.ez_gfx_gpu_heap_free_allocation(
		&heap,
		gfx.Ez_Gfx_Vertex_Allocation{start_index = 0, count = 4},
	)
	testing.expect_value(t, freed_status, gfx.Ez_Gfx_Status.Ok)

	reused, reused_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 8)
	testing.expect_value(t, reused_status, gfx.Ez_Gfx_Status.Ok)
...
vertex_allocator_merges_adjacent_free_chunks :: proc(t: ^testing.T) {
	...
	_, a_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 8)
	_, b_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 8)
	_, c_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 8)
	testing.expect_value(t, a_status, gfx.Ez_Gfx_Status.Ok)
	testing.expect_value(t, b_status, gfx.Ez_Gfx_Status.Ok)
	testing.expect_value(t, c_status, gfx.Ez_Gfx_Status.Ok)

	allocations := [?]gfx.Ez_Gfx_Vertex_Allocation {
	...
	for allocation in allocations {
		testing.expect_value(
			t,
			gfx.ez_gfx_gpu_heap_free_allocation(&heap, allocation),
...
vertex_allocator_recovers_capacity_after_free :: proc(t: ^testing.T) {
	...
	full_offset, full_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 16)
	overflow_offset, overflow_status := gfx.ez_gfx_gpu_heap_allocate_range(&heap, 4)
	testing.expect_value(t, full_status, gfx.Ez_Gfx_Status.Ok)
	testing.expect_value(t, full_offset, vk.DeviceSize(0))
	testing.expect_value(t, overflow_status, gfx.Ez_Gfx_Status.Native_Failure)
...
vertex_allocator_reserves_range_without_cpu_upload :: proc(t: ^testing.T) {
	...
	allocation, offset, status := gfx.ez_gfx_gpu_heap_reserve_allocation(&heap, 12, 3)
	...
	_, _, inconsistent_status := gfx.ez_gfx_gpu_heap_reserve_allocation(&heap, 12, 1)
	testing.expect_value(t, inconsistent_status, gfx.Ez_Gfx_Status.Invalid_Argument)
	testing.expect_value(t, heap.high_water, vk.DeviceSize(12))
}
