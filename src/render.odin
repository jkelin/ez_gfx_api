...
begin_render :: proc(window: ^Ez_Gfx_Window) -> bool {
	ctx := get_current_ctx()
	render := &ctx.render
	render^ = {}
	render.ctx = ctx
	...
	if !ctx_wait_timeline(ctx, render.frame.last_submitted_timeline) {
		render.active = false
		return false
	}
	render.texture_upload_wait_timeline = texture_manager_latest_scheduled_timeline(
		&ctx.texture_manager,
	)
	if !texture_manager_wait_submitted_timeline(
		&ctx.texture_manager,
	...
	render.vertex_upload_wait_timeline = vertex_manager_latest_scheduled_timeline(
		&ctx.vertex_manager,
	)
	if !vertex_manager_wait_submitted_timeline(
		&ctx.vertex_manager,
	...
	texture_manager_collect_destroyed(&ctx.texture_manager, ctx)
	vertex_manager_collect_completed(&ctx.vertex_manager)
	indirect_buffer_manager_release_completed(&ctx.indirect_manager)
	structured_buffer_manager_release_completed(&ctx.structured_buffer_manager)
	...
	if !ctx_wait_timeline(ctx, swapchain.last_write_timeline[render.image_index]) {
		render.active = false
		return false
	}
...
render_add_vertex_pipeline :: proc {
	render_add_vertex_pipeline_without_push_constants,
	render_add_vertex_pipeline_with_dynamic_state_without_push_constants,
	render_add_vertex_pipeline_with_dynamic_state_and_push_constants,
}

render_add_compute_pipeline :: proc {
	render_add_compute_pipeline_without_push_constants,
	render_add_compute_pipeline_with_push_constants,
}

render_acquire_indirect_buffer :: proc(
	$T: typeid,
	element_count: u32,
	debug_name: cstring,
	...
	indirect, indirect_ok := indirect_buffer_manager_acquire(
...
render_add_vertex_pipeline_without_push_constants :: proc(
...
render_add_vertex_pipeline_with_dynamic_state_without_push_constants :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	indirect: Ez_Gfx_Indirect_Buffer_Handle,
	bindings: []Ez_Gfx_Render_Binding,
...
render_add_vertex_pipeline_with_dynamic_state_and_push_constants :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	indirect: Ez_Gfx_Indirect_Buffer_Handle,
	bindings: []Ez_Gfx_Render_Binding,
	dynamic_state: Ez_Gfx_Render_Dynamic_State,
...
render_add_vertex_pipeline_impl :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	indirect: Ez_Gfx_Indirect_Buffer_Handle,
	bindings: []Ez_Gfx_Render_Binding,
	dynamic_state: Ez_Gfx_Render_Dynamic_State,
	...
	if !render_target_manager_acquire_shader_targets_for_bindings(
	...
	pipeline, pipeline_ok := pipeline_manager_get(
	...
	if !render_graph_add_vertex_pipeline(
		&render.graph,
		descriptor,
		shader,
		&render.ctx.render_target_manager,
	...
	version := pipeline_descriptor_version(render.ctx, shader, node)
	if pipeline.descriptor_versions[descriptor.descriptor_set_index] != version {
		if !pipeline_update_descriptors(
			render.ctx,
			pipeline,
			shader,
...
render_add_compute_pipeline_without_push_constants :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	dispatch_x, dispatch_y, dispatch_z: u32,
	bindings: []Ez_Gfx_Render_Binding,
) -> Ez_Gfx_Compute_Pipeline_Descriptor {
	return render_add_compute_pipeline_impl(
		shader,
		dispatch_x,
		dispatch_y,
		dispatch_z,
...
render_add_compute_pipeline_with_push_constants :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	dispatch_x, dispatch_y, dispatch_z: u32,
	bindings: []Ez_Gfx_Render_Binding,
	push_constants: $T,
...
render_add_compute_pipeline_impl :: proc(
	shader: ^Ez_Gfx_Shader_Program,
	dispatch_x, dispatch_y, dispatch_z: u32,
	bindings: []Ez_Gfx_Render_Binding,
	push_constant_data: rawptr,
	...
	pipeline, pipeline_ok := compute_pipeline_manager_get(
		&render.ctx.pipeline_manager,
		shader,
	)
	if !pipeline_ok do return {}
	...
	if !render_graph_add_compute_pipeline(&render.graph, descriptor, shader, render, bindings) {
		return {}
	}
	node := &render.graph.nodes[render.graph.node_count - 1]
	version := pipeline_descriptor_version(render.ctx, shader, node)
...
finish_render :: proc() -> bool {
	render := &get_current_ctx().render
	if !render_graph_validate(render) {
		render^ = {}
		return false
	}
	if !render_graph_execute(render) {
		// A failed graph must not leave the thread-local render active for the next frame.
		render^ = {}
		return false
	}

	ok := render_submit_and_present(render)
	render^ = {}
	return ok
}

render_submit_and_present :: proc(render: ^Ez_Gfx_Render) -> bool {
	window := render.window
	swapchain := &window.swapchain
	swapchain.last_write_timeline[render.image_index] = render.timeline_end

	if window.cache_presented_snapshots {
		if !ctx_wait_timeline(render.ctx, render.timeline_end) {
			return false
		}
		if !swapchain_cache_presented_snapshot(swapchain, render.image_index) {
			return false
		}
	}
...
image_layout_src_access :: proc(layout: vk.ImageLayout) -> vk.AccessFlags2 {
...
image_layout_src_stage :: proc(layout: vk.ImageLayout) -> vk.PipelineStageFlags2 {
	#partial switch layout {
	case .UNDEFINED:
		return {.TOP_OF_PIPE}
	case .TRANSFER_DST_OPTIMAL:
...
transition_image_with_aspect :: proc(
	command_buffer: vk.CommandBuffer,
	image: vk.Image,
	old_layout, new_layout: vk.ImageLayout,
	src_access, dst_access: vk.AccessFlags2,
