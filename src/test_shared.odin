...
example_glfw_init :: proc() -> bool {
	...
	glfw.WindowHint(glfw.VISIBLE, !ez_gfx_config_hidden_window())
	return true
}
...
example_window_create :: proc(
	...
	desc.cache_presented_snapshots = ez_gfx_config_screenshot_enabled()
	surface, status := ez_gfx_surface_create(ctx, desc)
	if status != .Ok {
...
example_window_poll_events :: proc(window: ^Example_Window) {
	for {
		...
		if window.framebuffer_resized {
			_ = ez_gfx_surface_resize(window.ctx, window.surface, u32(width), u32(height))
...
example_window_destroy :: proc(window: ^Example_Window) {
	...
	if window.ctx != 0 && window.surface != 0 {
		_ = ez_gfx_surface_destroy(window.ctx, window.surface)
	}
	if handle != nil {
		glfw.DestroyWindow(handle)
	}
