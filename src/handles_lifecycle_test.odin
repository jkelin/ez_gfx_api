...
@(private)
test_context_insert :: proc(t: ^testing.T) -> (Ez_Gfx_Context_Handle, ^Ez_Gfx_Ctx, Handle) {
	...
	arena_destroy_impl(&context_arena)
	context_arena_ready = false
	if !testing.expect(t, ensure_context_arena()) do return 0, nil, {}
	local, err = arena_insert_impl(&context_arena, Ez_Gfx_Ctx{})
	if !testing.expect_value(t, err, Error.None) do return 0, nil, {}
	ctx, get_err := arena_get_impl(&context_arena, local)
	if !testing.expect_value(t, get_err, Error.None) do return 0, nil, {}
	ctx.local_handle = local
...
handles_stale_context_reuse_fails :: proc(t: ^testing.T) {
	...
	reused_local, err := arena_insert_impl(&context_arena, Ez_Gfx_Ctx{})
	if !testing.expect_value(t, err, Error.None) do return
	defer {
		if reused_ctx, get_err := arena_get_impl(&context_arena, reused_local); get_err == .None do ctx_child_arenas_destroy(reused_ctx)
		_ = arena_remove_impl(&context_arena, reused_local)
	}
...
handles_child_owner_and_cross_context_fail_closed :: proc(t: ^testing.T) {
	...
	child_local, err := arena_insert_impl(&owner.surface_arena, Ez_Gfx_Window{native_window = rawptr(uintptr(1)), surface_platform = u32(EZ_GFX_SURFACE_PLATFORM_GLFW), framebuffer_width = 1, framebuffer_height = 1})
	if !testing.expect_value(t, err, Error.None) do return
	defer _ = arena_remove_impl(&owner.surface_arena, child_local)
...
handles_cross_kind_reuse_fails_closed :: proc(t: ^testing.T) {
	...
	surface_local, surface_err := arena_insert_impl(&owner.surface_arena, Ez_Gfx_Window{native_window = rawptr(uintptr(2)), surface_platform = u32(EZ_GFX_SURFACE_PLATFORM_GLFW), framebuffer_width = 1, framebuffer_height = 1})
	if !testing.expect_value(t, surface_err, Error.None) do return
	defer _ = arena_remove_impl(&owner.surface_arena, surface_local)
...
handles_context_destruction_invalidates_children :: proc(t: ^testing.T) {
	...
	if owner == nil do return
	child_local, err := arena_insert_impl(&owner.shader_arena, Ez_Gfx_Shader_Program{})
	if !testing.expect_value(t, err, Error.None) do return
