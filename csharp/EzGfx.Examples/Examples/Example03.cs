...
public static class Example03
{
    public static void Run(ExampleOptions options)
    {
        ...
        using GraphicsWindow window = graphics.CreateWindow(
            "ez_gfx_api compute structured buffer",
            1280,
            720,
            options.Hidden,
        ...
        using ShaderProgram computeShader = graphics.CompileShader(new ShaderDescription(
            ExampleHost.ShaderPath("3_compute_structured_buffer", "compute.slang"),
            EzGfx.Native.EzGfxShaderKind.Compute));
        using ShaderProgram drawShader = graphics.CompileShader(new ShaderDescription(
            ExampleHost.ShaderPath("3_compute_structured_buffer", "draw.slang")));
        ...
        graphics.CreateIndexHeap(vertices, indexBytes + 4096, "example 3 index heap");
        ...
        Matrix4x4 view = ExampleHost.OrbitView(-30, 52, 2.2f, 0, 0.55f, 0);
        Matrix4x4 projection = ExampleHost.VulkanPerspective(60, 1280f / 720f, 0.1f, 500f);
        byte[] drawPush = ExampleHost.MatrixPush(view * projection);
        byte[] primitiveBytes = PackExample3Records(records);
        ExampleHost.RunFrames(graphics, window, options.Frames, _ =>
        {
            using IndirectBuffer indirect = graphics.AcquireIndirect((uint)records.Count, "example 3 draw commands");
    ...
    private static byte[] PackExample3Records(IReadOnlyList<PrimitiveRecord> records)
    {
